...
iptables -t nat -I PREROUTING  -s $VPNIPPOOL -p tcp -m multiport --dport 80 -j DNAT --to-destination 192.168.50.5:3128
iptables -t nat -I PREROUTING  -s $VPNIPPOOL -p tcp -m multiport --dport 443 -j DNAT --to-destination 192.168.50.5:3128
...
# test out removing these rules to see if it fixes the issue with squid
